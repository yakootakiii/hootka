...
async function main() {
  ...
  if (command === 'open') {
    ...
    const { data } = await httpsCallable<any, any>(getFunctions(app), 'createGame')({ quizId });
    console.log(`CODE=${data.code} GAME=${data.gameId}`);
  } else if (command === 'advance') {
    const { data } = await httpsCallable<any, any>(getFunctions(app), 'advanceGame')({ gameId: arg });
